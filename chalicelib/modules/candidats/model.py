...
class MetaModel(type(BaseModel)):
    def __getattr__(cls, item):
        ...
        if item == "id" and cls == "CandidatModel":
            return "_id"
...
class CandidatModel(BaseModel, metaclass=MetaModel):
    """Candidat model for MongoDB collection"""
...
class CandidatPatchModel(BaseModel, metaclass=MetaModel):
    """Candidat patch model for partial updates"""
...
class CandidatModelResult(BaseModel, metaclass=MetaModel):
    """Candidat result model with pagination"""
    model_config = ConfigDict(populate_by_name=True, arbitrary_types_allowed=True)
    
    total: int
    results: list[CandidatModel]
