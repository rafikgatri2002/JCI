...
def get_all_with_criteria(
    ...
    # If userId is provided, get specific candidat
    if userId:
        model = service.get_by_user(userId)
        if model:
...
@router.patch('/{_id}')
@exception_handler
def patch(_id: str = Path(...), candidat_patch_model: CandidatPatchModel = Body(...)):
